...
- `src/game/ConstantsTower.ts`: `TowerVariantStats` (type alias at **line 144**) currently has `knockback: boolean` (**line 158**) — replace with `knockbackBase: number` and `knockbackScale: number` (default `0`; `knockbackBase === 0` ⟺ disabled, identical to the old `false`).
   - **Revision (post-draft):** the standalone per-tower knockback constants `RAILGUN_KNOCKBASE`, `RAILGUN_KNOCK_SCALE`, `SHOTGUN_KNOCKBASE`, `SHOTGUN_KNOCK_SCALE` were **removed** — their values live only in `TOWER_BASE` (`railgun.knockbackBase/Scale`, `shotgunTank.knockbackBase/Scale`). A new generic `settings?: Partial<TowerBase>` field was added to `TowerVariantConfig` so **any** variant can override **any** `TowerBase` field declaratively. railgun A uses `settings: { knockbackBase: TOWER_BASE.railgun.knockbackBase! * RAILGUN_KNOCKBACK_MULT, knockbackScale: TOWER_BASE.railgun.knockbackScale! }` (the `× RAILGUN_KNOCKBACK_MULT` amplification, `RAILGUN_KNOCKBACK_MULT` retained in `TOWER_BASE` scope as the only remaining knockback constant), and shotgunTank B uses `settings: { knockbackBase: TOWER_BASE.shotgunTank.knockbackBase!, knockbackScale: TOWER_BASE.shotgunTank.knockbackScale! }`. `apply` on `TowerVariantConfig` is now optional (static overrides move into `settings`; dynamic/tier-based tweaks keep `apply`).
