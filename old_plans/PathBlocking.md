...
- `src/game/ConstantsTower.ts`: `TowerVariantStats` (type alias at **line 144**) currently has `knockback: boolean` (**line 158**) — replace with `knockbackBase: number` and `knockbackScale: number` (default `0`; `knockbackBase === 0` ⟺ disabled, identical to the old `false`).
   - **Revision (post-draft):** the standalone per-tower knockback constants `RAILGUN_KNOCKBASE`, `RAILGUN_KNOCK_SCALE`, `SHOTGUN_KNOCKBASE`, `SHOTGUN_KNOCK_SCALE` and the `RAILGUN_KNOCKBACK_MULT` amplifier were **removed** — their values live only in `TOWER_BASE` (`railgun.knockbackBase/Scale`, `shotgunTank.knockbackBase/Scale`). A new generic `settings?: Partial<TowerBase>` field was added to `TowerVariantConfig` so **any** variant can override **any** `TowerBase` field declaratively with a literal. `apply` on `TowerVariantConfig` is now optional (static overrides move into `settings`; dynamic/tier-based tweaks keep `apply`). Concretely: railgun A uses `settings: { knockbackBase: 0.9 }` (the old `0.3 × RAILGUN_KNOCKBACK_MULT`/×3 amplification, now a literal) and shotgunTank B ("Repulsor") uses `settings: { knockbackBase: 0.9 }` (2× the `0.45` base). The only remaining knockback constant is `KNOCKBACK_HP_DIVISOR` (renamed from `RAILGUN_KNOCK_HP_DIVISOR`; it already applies to **all** knockback projectiles, not just railgun).
...
- `src/game/ProjectileManager.ts`: `spawn` opts are `knockbackBase?: number; knockbackScale?: number` (resolved from the passed values, falling back to `TOWER_BASE[towerType]` defaults). Knockback is "enabled" when `knockbackBase > 0`, and `applyProjectileEffects` derives `projectile.knockback = knockbackBase + knockbackScale * tier`. The old railgun-specific knockback block and the broken `if (knockback)` gate were removed — the generic `knockbackBase > 0` path now drives all knockback uniformly (railgun and shotgun alike). The knockback magnitude is HP-scaled via `KNOCKBACK_HP_DIVISOR` (clamped 0.1×–2×) inside the `projectile.knockback > 0` apply block, which runs for any knockback projectile. No other behavioral change.
...
- `src/game/ConstantsTower.ts`:
  ...
  - `TOWER_BASE` (**line 47**):
    ...
    - shotgunTank: `range: 1`, `damage: 8` (like basic), `fireRate: 1.2`, `projSpeed: 14`, `health: 250` (independent static value like the other towers — the "basicHealth * 10" phrasing in the draft was only a planning reference, not a code dependency). shotgunTank also carries a default `knockbackBase: 0.45` / `knockbackScale: 0.3` (the old `SHOTGUN_KNOCKBASE = RAILGUN_KNOCKBASE * 1.5` = 0.45 value, now expressed directly in `TOWER_BASE`).
  ...
  - `TOWER_VARIANTS` (**line 166**):
    ...
    - shotgunTank B ("Repulsor"): knockback via `settings: { knockbackBase: TOWER_BASE.shotgunTank.knockbackBase, knockbackScale: TOWER_BASE.shotgunTank.knockbackScale }` (uses the 5a base/scale pair, sourced from `TOWER_BASE`).
...
- `src/towers/Tower.ts`:
  ...
  - Shotgun Tank A +health: apply in `constructor`/variant application by scaling `maxHealth`/`health`.
   - Shotgun Tank B knockback: already wired through `_computeStats` + the B-variant `settings` override, so `fire()` passes `stats.knockbackBase`/`stats.knockbackScale` (the 5a pair) with no special-casing and no `SHOTGUN_KNOCK*` constants.
